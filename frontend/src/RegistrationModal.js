        ...
        return (
          <Form>
            ...
            <FormGroup>
              ...
              <Input type="text" name="dlNumber" id="dlNumber" value={formData.dlNumber} onChange={handleInputChange} />
              {errors.dlNumber && <div className="error">{errors.dlNumber}</div>}
            </FormGroup>
            <FormGroup>
        ...
        return (
          <Form>
            <FormGroup>
              ...
              <Input type="select" name="role" id="role" value={formData.role} onChange={handleInputChange}>
            ...
            {['DMV', 'Insurance', 'Service Center', 'Dealership'].includes(formData.role) && (
              <FormGroup>
                <Label for="additionalId">{`${formData.role} ID`}</Label>
                <Input type="text" name="additionalId" id="additionalId" value={formData.additionalId} onChange={handleInputChange} />
              </FormGroup>
            )}
          </Form>
        );
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle}>
      ...
      <div className="modal-footer">
        {step > 1 && <Button color="secondary" onClick={previousStep}>Back</Button>}
        {step < 2 ? <Button color="primary" onClick={nextStep}>Next</Button> : <Button color="success" onClick={handleSubmit}>Submit</Button>}
      </div>
    </Modal>
  );
};
